...
TOR_INSTANCES=${TOR_INSTANCES:=2 }
TOR_CONTROLLER_HOST=${TOR_CONTROLLER_HOST}
TOR_HASHED_PASSWORD=${TOR_HASHED_PASSWORD}

# if defined TOR_OPTIONSE env variable can be used to add options to TOR
TOR_OPTIONS=${TOR_OPTIONS:=''}

echo $TOR_HASHED_PASSWORD
...
do
        ...
        echo "Creating $j $control_port $socks_port"
        ...
        tor --RunAsDaemon 1 --CookieAuthentication 0 --HashedControlPassword $TOR_HASHED_PASSWORD --PidFile $dir_data/tor$i/tor$i.pid --SocksPort 0.0.0.0:$socks_port --DataDirectory $dir_data/tor$i --ControlPort 0.0.0.0:$control_port
done
